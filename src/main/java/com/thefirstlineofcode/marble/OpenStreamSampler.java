...
import com.thefirstlineofcode.marble.im.SubscriptionManager;
import com.thefirstlineofcode.marble.im.SubscriptionManager.Action;
...
public class OpenStreamSampler extends AbstractXmppSampler {
	...
	public static final String SENDING_INITIAL_PRESENCE = "OpenStreamSampler.sendingInitialPresence";
	
	private static final Logger logger = LoggerFactory.getLogger(OpenStreamSampler.class);
			...
			for (JMeterTreeNode node : nodes) {
				...
				if (plugins != null && plugins.length != 0) {
					for (Class<?> contributedPlugin : plugins) {
						...
						if (logger.isDebugEnabled())
							logger.debug("register plugin {} to chat client", plugin);
					}
				}
			}
	...
	public void setSendingInitialPresence(boolean sendingInitialPresence) {
		setProperty(new BooleanProperty(SENDING_INITIAL_PRESENCE, sendingInitialPresence));
	}
	
	public boolean isSendingInitialPresence() {
		return getPropertyAsBoolean(SENDING_INITIAL_PRESENCE, true);
	}
	...
	private class OpenStreamListener implements IConnectionListener, INegotiationListener {
		...
		public void messageReceived(String message) {
			...
			result.setBytes(result.getBytesAsLong() + message.getBytes().length);
		}
