package com.thefirstlineofcode.marble.im;
...
public class MessageSampler extends AbstractXmppSampler implements IPluginContributor {
	private static final long serialVersionUID = 4109484542472338833L;
	
	private static final String RESOURCE_MISSION_STATEMENTS = "com/thefirstlineofcode/marble/resources/mission_statements.txt";
	...
	private static String[] loadMissionStatements() {
		...
		try {
			...
			throw new RuntimeException(String.format("can't load mission statements from resource: '%'.",
					RESOURCE_MISSION_STATEMENTS), e);
		} finally {
			if (reader != null) {
				try {
					reader.close();
